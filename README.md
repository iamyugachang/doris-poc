...
```
Client VM（同 VPC）三個 probe 同時跑，每秒一步 INSERT→UPSERT→SELECT→DELETE→SELECT，寫入失敗後另開連線做讀取檢查
...
| 設定 | **Ansible**（`ansible/`） | 機器裡「長什麼樣」：JDK、Doris 安裝與 conf、systemd unit、FE/BE 註冊成叢集、client VM 的三個 probe | 開關機、製造故障、量測 |
...
- `group_vars/all.yml`：Doris 版本與下載網址、路徑、`priority_networks`、FE heap、BE mem_limit、probe 指令
- 可重跑：第二次執行 `changed=0`；`--check --diff` 可預演；`--limit client` 只重佈 probe
...
| `./demo.sh status` | FE / BE 存活數、master、replica 狀態、目前製造故障中的故障 |
| `./demo.sh monitor [--fresh] [--order master-first\|follower-first]` | 在 client VM 起三個 probe 並合併即時捲動 |
| `./demo.sh break [--target fe\|be\|vm] [--mode crash\|dead\|hang\|stop] [--on master\|follower\|<vm>]` | 製造故障（預設停 master 所在 VM）；可連續執行疊加 |
| `./demo.sh restore` | 依相反順序還原全部故障（VM start / systemctl start / kill -CONT），等叢集健康 |
| `./demo.sh measure [--json] [--label X]` | 拉回三份 log：失敗時間、無回應時間、可用性等級（讀寫正常 / 只讀 / 不可用 / 卡住） |
| `./demo.sh stop` / `start` | terraform vm_status 切換；start 後自動回到叢集、確保測試表 |
...
| `experiments/fill_results.py` | 從 log 重算 3 輪結果 → `results/summary.json`（results.html 與 index.html 的資料來源；不寫進 spec） |
...
| 故障（3 輪最差等級） | mysql | jdbc | arrow-flight |
|---|---|---|---|
...
| 交錯（VM 停 + BE dead + FE dead） | 只讀（僅存的 follower FE 仍回 SELECT） | 不可用 | 讀取失敗 |

48 次執行 0 筆資料不一致；每次 restore 後都回到 3 FE / 3 BE、12 個 replica 正常。中斷秒數 = 失敗時間（第一次失敗到下一次成功）與無回應時間（連續操作間隔 > 5 秒）的聯集，含 client 自己的 timeout（connect 3s、read/write 8s）。本次 asia-east1-c 缺 e2-standard-4，doris-3 以 n2 / n2d-standard-4 執行（見 `results/run-notes.md`）；上一次（2026-09-11）結果在 `results/archive/`。
...
clients/        三個 probe 程式（部署到 client VM） experiments/    scenarios.yaml、run_matrix.py、fill_results.py、finalize.sh
openspec/       規格（主 spec + change）      results/        3 輪原始結果與彙整
